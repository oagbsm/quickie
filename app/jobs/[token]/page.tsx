...
import { chooseMarketplaceQuote, submitMarketplaceOffer } from "@/app/jobs/actions";

export const metadata: Metadata = { title: "Quickola job", robots: { index: false, follow: false } };

export default async function JobPage({ params, searchParams }: { params: Promise<{ token: string }>; searchParams: Promise<{ error?: string; offered?: string }> }) {
  ...
  return <main className="min-h-screen bg-[#f7f8fa] text-[#061b3f]"><MarketplaceHeader /><section className="mx-auto max-w-3xl px-5 py-10 sm:px-8 sm:py-16"><div className="rounded-3xl border border-[#e7ebef] bg-white p-6 sm:p-9"><p className="text-xs font-black uppercase tracking-[.16em] text-[#159548]">{isOwner ? "Your job" : "Local job"}</p><h1 className="mt-3 text-4xl font-black tracking-[-.06em]">{title}</h1><p className="mt-4 text-sm text-[#526078]">📍 {data.postcode} · {data.requested_timing || "Flexible timing"}</p>{data.optional_note && <p className="mt-6 whitespace-pre-wrap leading-7 text-[#39465b]">{data.optional_note}</p>}<div className="mt-7 grid gap-3 sm:grid-cols-3"><Detail label="Budget" value={price} /><Detail label="Offers" value={`${quotes?.length || 0}`} /><Detail label="Status" value={data.status.replaceAll("_", " ")} /></div>{!isOwner && user && <form action={submitMarketplaceOffer} className="mt-8 rounded-2xl bg-[#eef8f1] p-5"><h2 className="text-xl font-black">Make an offer</h2><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-sm font-black">Your price (£)<input name="amount" type="number" min="1" step="1" required className="mt-2 min-h-12 w-full rounded-xl border border-[#dbe1ea] bg-white px-4" /></label><label className="text-sm font-black">When can you do it?<input name="availability" defaultValue="Flexible" className="mt-2 min-h-12 w-full rounded-xl border border-[#dbe1ea] bg-white px-4" /></label></div><label className="mt-3 block text-sm font-black">Message<textarea name="message" rows={3} placeholder="Hi, I can help with this job." className="mt-2 w-full rounded-xl border border-[#dbe1ea] bg-white px-4 py-3" /></label><input type="hidden" name="token" value={token} /><button className="mt-4 min-h-12 rounded-xl bg-[#23a955] px-6 font-black text-[#061b3f]">{query.offered === "1" ? "Offer sent" : "Send offer"}</button>{query.error && <p className="mt-3 text-sm font-bold text-red-700">We couldn’t send that offer. Please try again.</p>}</form>}{!user && <p className="mt-8 rounded-2xl bg-[#f7f8fa] p-5 text-sm font-bold">Want to make an offer? <a className="text-[#167d3c]" href={`/sign-in?next=${encodeURIComponent(`/jobs/${token}`)}`}>Sign in first →</a></p>}{isOwner && <section className="mt-8 border-t border-[#e9edf1] pt-7"><h2 className="text-2xl font-black">Offers received</h2>{quotes?.length ? <div className="mt-4 grid gap-3">{quotes.map((quote) => <article key={quote.id} className="rounded-2xl bg-[#f7f8fa] p-5"><div className="flex items-start justify-between gap-4"><div><p className="font-black">{(quote.cleaner_profiles as { business_name?: string | null; display_name?: string | null } | null)?.business_name || (quote.cleaner_profiles as { display_name?: string | null } | null)?.display_name || "Local person"}</p><p className="mt-1 text-sm text-[#526078]">{quote.availability_text || "Flexible"} · {quote.status}</p></div><p className="text-2xl font-black">£{Math.round(quote.amount_pence / 100)}</p></div>{quote.message && <p className="mt-3 text-sm leading-6 text-[#526078]">{quote.message}</p>}{["pending", "submitted"].includes(quote.status) && <form action={chooseMarketplaceQuote} className="mt-4"><input type="hidden" name="token" value={token} /><input type="hidden" name="quoteId" value={quote.id} /><button className="min-h-11 rounded-xl bg-[#061b3f] px-5 text-sm font-black text-white">Accept offer</button></form>}</article>)}</div> : <p className="mt-3 text-[#657089]">No offers yet. We’ll show them here as people respond.</p>}</section>}</div></section></main>;
}
