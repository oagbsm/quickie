...
function JobDetails({ data, photos, selected }: { data: { service: string; service_subtype: string | null; pricing_answers: unknown; postcode: string; requested_timing: string | null; optional_note: string | null; budget_amount: number | null }; photos: Array<{ id: string; url: string | null }>; selected: boolean }) {
  ...
  return <section className="mt-9 border-t border-[#edf0f3] pt-7">
    ...
    <p className="mt-2 text-sm font-bold text-[#526078]">{data.budget_amount == null ? (selected ? "Payment required" : "Open to offers") : `Budget · £${Number(data.budget_amount).toFixed(2).replace(/\\.00$/, "")}`}</p>
