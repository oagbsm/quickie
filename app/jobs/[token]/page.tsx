...
function BookingConfirmation({ token, jobId, booking, quote, lifecycle, providerName, providerId, providerPhotoUrl, schedule, review }: { token: string; jobId: string; booking: { id: string; conversation_id?: string | null; amount_pence: number; payment_status?: string | null; stripe_checkout_session_id?: string | null; status?: string | null; completion_status?: string | null; scheduled_date?: string | null; arrival_window_start?: string | null; arrival_window_end?: string | null }; quote: { id: string; amount_pence: number }; lifecycle: ReturnType<typeof getCustomerJobLifecycleState>; providerName: string; providerId: string | null; providerPhotoUrl?: string | null; schedule: ReturnType<typeof formatMarketplaceSchedule>; review: { rating: number; review_text: string | null } | null }) {
  ...
  return <section className="mt-7 rounded-2xl border border-[#cfe9d7] bg-[#f5fbf6] p-5 sm:p-6">
    ...
    {lifecycle === "completed" && review && <p className="mt-4 font-black text-[#167d3c]">Your review: {"★".repeat(review.rating)}</p>}
  </section>;
}
