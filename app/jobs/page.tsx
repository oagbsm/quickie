...
export default async function JobsPage() {
  ...
  const { data: jobs } = await admin.from("marketplace_jobs").select("id,public_token,service,service_subtype,postcode,optional_note,requested_timing,map_latitude,map_longitude,created_at,marketplace_quotes(count)").in("status", ["posted", "finding_provider"]).order("created_at", { ascending: false }).limit(80);
