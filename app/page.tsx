...
const features = [
  ["building", "Manage every property", "Keep addresses, access instructions and cleaning requirements together in one place."],
  ["calendar", "Request cleans quickly", "Choose a property, service, date and frequency through one simple booking flow."],
  ["document", "Follow every booking", "See when a booking is received, confirmed, assigned, in progress and completed."],
] as const;

const steps = [
  ["building", "01", "Add your properties", "Save addresses, property details and access instructions."],
  ["calendar", "02", "Request a clean", "Choose the property, service, date and frequency."],
  ["clean", "03", "Quickola arranges the service", "We assign and manage the cleaner for the booking."],
  ["chart", "04", "Follow the progress", "Track the booking from receipt through completion."],
] as const;
  ...
  return (
    <div className={styles.page}>
        ...
        <main id="main-content">
          <div className={styles.hero}>
            <div className={styles.heroCopy}>
              ...
              <h1><span className={refinements.heroLine}>The smarter way to</span>{" "}<br />manage every clean,{" "}<br /><span className={refinements.greenText}>every time.</span></h1>
              ...
              <div className={styles.heroActions}>
                <Link href="/business/enquire" className={`${styles.primaryButton} ${refinements.primaryCta}`}>Request business access <span>→</span></Link>
              ...
              <div className={styles.heroBenefits}>
                ...
                <Benefit icon="target" title="Booking updates" text="Clear lifecycle" />
                <Benefit icon="shield" title="Managed service" text="Quickola coordinates" />
              </div>
      ...
      <section className={styles.featuresSection} id="for-businesses">
        <div className={styles.featuresWrap}>
          <div className={styles.featuresLead}>
            ...
            <h2>Everything you need to organise property cleaning in one place.</h2>
          </div>
          <p className={styles.featuresIntro}>Keep property details, cleaning requests and booking progress together without relying on separate message threads.</p>
      ...
      <section className={styles.processSection} id="how-it-works">
        <div className={styles.processWrap}>
          ...
          <div className={`${styles.ctaPanel} ${refinements.ctaPanel}`}>
            <div><h2>Ready to simplify your property cleaning?</h2><p>Add your properties, request cleans and follow every booking from one place.</p></div>
            <div className={styles.ctaActions}><Link href="/business/enquire" className={styles.ctaLight}>Request business access <span>→</span></Link><Link href="#how-it-works" className={styles.ctaDark}>See how it works <PlayIcon /></Link></div>
          </div>
        </div>
      </section>
      ...
      <footer className={styles.trustStrip}>
        ...
        <Trust icon="shield" title="Managed service" text="Assignment by Quickola" />
        <Trust icon="pin" title="Serving Slough" text="Controlled pilot area" />
        <Trust icon="building" title="MATO GROUP LTD" text="Company number 17327292" />
      </footer>
    </div>
  );
}
...
function HomeHeader() {
  const links = [["How it works", "#how-it-works"], ["For businesses", "#for-businesses"]];
  return <header className={styles.header}><Link href="/" className={styles.logo} aria-label="Quickola homepage"><Image src="/quickola/logo-mark.png" alt="" width={30} height={31} priority /><span>Quickola</span></Link><nav aria-label="Primary navigation">{links.map(([label, href]) => <Link key={href} href={href}>{label}</Link>)}</nav><div className={styles.headerActions}><Link href="/business/sign-in">Sign in</Link><Link href="/business/enquire" className={refinements.headerCta}>Request business access <span>→</span></Link></div><details className={styles.mobileMenu}><summary>Menu</summary><nav aria-label="Mobile navigation">{links.map(([label, href]) => <Link key={href} href={href}>{label}</Link>)}<Link href="/business/sign-in">Sign in</Link><Link href="/business/enquire">Request business access</Link></nav></details></header>;
}
