...
export default async function MyJobsPage() {
  ...
  const extendedSelect = "id,public_token,service,service_subtype,postcode,budget_amount,status,created_at,marketplace_quotes(id,amount_pence,status),marketplace_bookings(id,quote_id,amount_pence,payment_status,stripe_checkout_session_id,paid_at,status,completion_status,scheduled_date,arrival_window_start,arrival_window_end)";
  const legacySelect = "id,public_token,service,service_subtype,postcode,budget_amount,status,created_at,marketplace_quotes(id,amount_pence,status),marketplace_bookings(id,quote_id,amount_pence,payment_status,stripe_checkout_session_id,paid_at)";
...
function DesktopJobCard({ item }: { item: JobView }) { const { job, activeQuotes, acceptedQuote, state, statusLabel, conversationIds, jobHref, title, amount, budget } = item; const isBooked = !["waiting_for_offers", "offers_received", "provider_selected_unpaid", "payment_pending"].includes(state); const activityHref = conversationIds.length === 1 ? `/messages/${conversationIds[0]}` : jobHref; return <article className="rounded-3xl border border-[#e7ebef] bg-white p-6"><Link href={jobHref} className="block"><div className="flex items-start justify-between gap-4"><div><h2 className="text-xl font-black capitalize">{title}</h2><p className="mt-2 text-sm text-[#657089]">{job.postcode}</p></div><span className={`rounded-full px-3 py-1 text-sm font-black ${isBooked ? "bg-[#dff4e4] text-[#167d3c]" : state === "provider_selected_unpaid" || state === "payment_pending" ? "bg-[#fff8e8] text-[#8a5a00]" : "bg-[#eef8f1] text-[#167d3c]"}`}>{statusLabel}</span></div>{acceptedQuote ? <div className={`mt-4 rounded-2xl p-4 ${isBooked ? "bg-[#eef8f1]" : "bg-[#f5fbf6]"}`}><p className="font-black text-[#167d3c]">{isBooked ? "✓ Booked" : "✓ Offer accepted"}</p><p className="mt-2 text-sm font-bold text-[#526078]">Agreed price</p><p className="text-2xl font-black">{formatMarketplaceAmount(amount)}</p>{state === "provider_selected_unpaid" && <><p className="mt-2 font-black text-[#8a5a00]">Payment required</p><p className="mt-1 text-sm text-[#526078]">Complete payment to confirm your booking.</p></>}{state === "payment_pending" && <p className="mt-2 font-black text-[#8a5a00]">Payment confirming</p>}{state === "completed" && <p className="mt-2 font-black text-[#167d3c]">✓ Job complete</p>}</div> : <><p className="mt-3 font-black text-[#167d3c]">Budget · {budget}</p><p className="mt-2 text-sm font-bold text-[#526078]">{activeQuotes.length} {activeQuotes.length === 1 ? "offer" : "offers"} received</p></>}</Link><div className="mt-4 flex flex-wrap items-center gap-3">{state === "provider_selected_unpaid" && acceptedQuote && <Link href={jobHref} className="rounded-xl bg-[#23a955] px-4 py-3 font-black text-[#061b3f]">Confirm &amp; pay {formatMarketplaceAmount(amount)}</Link>}{isBooked && conversationIds.length === 1 && <Link href={activityHref} className="font-black text-[#167d3c]">Message provider →</Link>}{!acceptedQuote && activeQuotes.length > 0 && <Link href={jobHref} className="font-black text-[#167d3c]">Review {activeQuotes.length === 1 ? "offer" : "offers"} →</Link>}{!acceptedQuote && activeQuotes.length === 0 && <span className="font-black text-[#657089]">Waiting for offers →</span>}{state === "payment_pending" && <Link href={jobHref} className="font-black text-[#167d3c]">View booking →</Link>}{acceptedQuote && !isBooked && state !== "payment_pending" && <Link href={activityHref} className="font-black text-[#167d3c]">View activity →</Link>}</div></article>; }
