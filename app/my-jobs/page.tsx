...
type JobView = { job: { id: string; public_token: string | null; service: string; service_subtype: string | null; postcode: string; requested_timing?: string | null; budget_amount: number | null }; activeQuotes: Quote[]; acceptedQuote?: Quote; booking?: Booking; providerName?: string; state: string; statusLabel: string; conversationIds: string[]; jobHref: string; title: string; amount?: number; budget: string };
...
function MobileJobCard({ item }: { item: JobView }) { const { job, activeQuotes, acceptedQuote, providerName, state, jobHref, title, amount, budget, conversationIds } = item; const isBooked = !["waiting_for_offers", "offers_received", "provider_selected_unpaid", "payment_pending"].includes(state); const metadataBudget = acceptedQuote ? `Agreed ${formatMarketplaceAmount(amount)}` : activeQuotes.length ? `${activeQuotes.length} ${activeQuotes.length === 1 ? "offer" : "offers"} received` : `Budget · ${budget}`; return <Link href={jobHref} className="relative block min-h-[154px] rounded-[22px] border border-[#e7ebef] bg-white p-5 shadow-[0_2px_10px_rgba(6,27,63,0.03)] transition-shadow hover:shadow-[0_8px_24px_rgba(6,27,63,0.06)] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#23a955]/30"><div className="pr-8"><h2 className="line-clamp-2 text-[19px] font-black leading-6 capitalize">{title}</h2><p className="mt-2 text-[14px] text-[#526078]">{job.postcode}{job.requested_timing ? ` · ${job.requested_timing}` : ""}</p></div><span aria-hidden="true" className="absolute right-5 top-1/2 -translate-y-1/2 text-3xl font-light leading-none text-[#061b3f]">›</span><div className="mt-6 border-t border-[#f0f2f5] pt-4 pr-8"><p className="text-[14px] font-black text-[#167d3c]">{getJobActivity(item)}</p><p className="mt-2 text-[14px] text-[#526078]">{metadataBudget}</p>{conversationIds.length > 0 && <p className="mt-2 text-[14px] font-bold text-[#526078]">{conversationIds.length} provider {conversationIds.length === 1 ? "conversation" : "conversations"}</p>}{isBooked && providerName && <p className="mt-2 text-[14px] font-black text-[#526078]">Booked with {providerName}</p>}</div></Link>; }
