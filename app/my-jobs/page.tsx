...
function DesktopJobCard({ item }: { item: JobView }) { const { job, activeQuotes, acceptedQuote, providerName, state, conversationIds, jobHref, title, amount, budget } = item; const isBooked = !["waiting_for_offers", "offers_received", "provider_selected_unpaid", "payment_pending"].includes(state); const conversationHref = conversationIds.length === 1 ? `/messages/${conversationIds[0]}` : `/messages?job=${job.id}`; const prices = activeQuotes.map((quote) => quote.amount_pence).filter(Number.isFinite); const priceRange = prices.length ? `${formatMarketplaceAmount(Math.min(...prices))}${prices.length > 1 ? ` – ${formatMarketplaceAmount(Math.max(...prices))}` : ""}` : null; return <article className="rounded-3xl border border-[#e7ebef] bg-white p-6 transition-shadow hover:shadow-[0_8px_28px_rgba(6,27,63,0.06)]"><Link href={jobHref} className="block focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#23a955]/30"><div className="flex items-start justify-between gap-4"><div><h2 className="text-xl font-black capitalize">{title}</h2><p className="mt-2 text-sm text-[#526078]">{job.postcode}{job.requested_timing ? ` · ${job.requested_timing}` : ""}</p></div><span className="text-right text-sm font-black text-[#167d3c]">{getJobActivity(item)}</span></div>{acceptedQuote ? <div className={`mt-5 border-t pt-4 ${isBooked ? "border-[#dcefe0]" : "border-[#f1e5c9]"}`}><p className="font-black text-[#167d3c]">{isBooked ? `Booked${providerName ? ` with ${providerName}` : ""}` : "Provider selected"}</p><p className="mt-2 text-sm font-bold text-[#526078]">Agreed price</p><p className="text-2xl font-black">{formatMarketplaceAmount(amount)}</p>{state === "provider_selected_unpaid" && <p className="mt-2 font-black text-[#8a5a00]">Payment required</p>}{state === "payment_pending" && <p className="mt-2 font-black text-[#8a5a00]">Payment confirming</p>}{state === "completed" && <p className="mt-2 font-black text-[#167d3c]">Job complete</p>}</div> : <div className="mt-5 flex flex-wrap items-end justify-between gap-4 border-t border-[#f0f2f5] pt-4"><div><p className="font-black text-[#167d3c]">{getJobActivity(item)}</p><p className="mt-2 text-sm text-[#526078]">{priceRange || `Budget · ${budget}`}</p></div></div>}</Link><div className="mt-5 flex flex-wrap items-center gap-4">{conversationIds.length > 0 && <Link href={conversationHref} className="min-h-11 py-2 font-black text-[#167d3c]">{conversationIds.length === 1 ? "View conversation" : `View ${conversationIds.length} conversations`} →</Link>}{state === "provider_selected_unpaid" && acceptedQuote && <Link href={jobHref} className="min-h-11 rounded-xl bg-[#23a955] px-4 py-3 font-black text-[#061b3f]">Confirm &amp; pay {formatMarketplaceAmount(amount)}</Link>}{isBooked && conversationIds.length === 1 && <Link href={conversationHref} className="min-h-11 py-2 font-black text-[#167d3c]">Message {providerName || "provider"} →</Link>}{!acceptedQuote && activeQuotes.length > 0 && <Link href={jobHref} className="min-h-11 py-2 font-black text-[#167d3c]">Review {activeQuotes.length === 1 ? "quote" : "quotes"} →</Link>}{!acceptedQuote && activeQuotes.length === 0 && <Link href={jobHref} className="min-h-11 py-2 font-black text-[#167d3c]">View job →</Link>}{state === "payment_pending" && <Link href={jobHref} className="min-h-11 py-2 font-black text-[#167d3c]">View booking →</Link>}{acceptedQuote && !isBooked && state !== "payment_pending" && <Link href={conversationHref} className="min-h-11 py-2 font-black text-[#167d3c]">View activity →</Link>}</div></article>; }

function MobileMyJobs({ jobs, error = false }: { jobs: JobView[]; error?: boolean }) { return <section className="px-4 pb-[calc(6.5rem+env(safe-area-inset-bottom))] pt-4"><h1 className="text-[34px] font-black tracking-[-.06em]">My jobs</h1><div className="mt-5 grid gap-4">{error ? <ErrorState /> : jobs.map((item) => <MobileJobCard key={item.job.id} item={item} />)}{!error && !jobs.length && <Empty text="You haven’t posted a job yet." />}</div><MobileBottomNav active="my-jobs" /></section>; }

function MobileJobCard({ item }: { item: JobView }) { const { job, activeQuotes, acceptedQuote, providerName, state, jobHref, title, amount, budget, conversationIds, latestMessageAt } = item; const isBooked = !["waiting_for_offers", "offers_received", "provider_selected_unpaid", "payment_pending"].includes(state); const metadataBudget = acceptedQuote ? `Agreed ${formatMarketplaceAmount(amount)}` : activeQuotes.length ? `${activeQuotes.length} ${activeQuotes.length === 1 ? "quote" : "quotes"} received` : `Budget · ${budget}`; const conversationHref = conversationIds.length === 1 ? `/messages/${conversationIds[0]}` : `/messages?job=${job.id}`; return <article className="rounded-[22px] border border-[#e7ebef] bg-white p-5 shadow-[0_2px_10px_rgba(6,27,63,0.03)] transition-shadow hover:shadow-[0_8px_24px_rgba(6,27,63,0.06)]"><Link href={jobHref} className="block focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#23a955]/30"><div className="flex items-start justify-between gap-3"><div className="min-w-0 flex-1"><h2 className="line-clamp-2 text-[19px] font-black leading-6 capitalize">{title}</h2><p className="mt-1 text-[14px] text-[#526078]">{job.postcode}{job.requested_timing ? ` · ${job.requested_timing}` : ""}</p></div><span aria-hidden="true" className="text-2xl font-light leading-none text-[#061b3f]">›</span></div><div className="mt-4 border-t border-[#f0f2f5] pt-4"><div className="flex items-center justify-between gap-3"><p className="rounded-full bg-[#eef8f1] px-3 py-1 text-[13px] font-black text-[#167d3c]">{getJobActivity(item)} {isBooked && "✓"}</p><p className="text-[14px] font-black text-[#526078]">{metadataBudget}</p></div>{isBooked && providerName && <div className="mt-4 flex items-center justify-between gap-3 text-[14px]"><span className="text-[#526078]">Booked with</span><span className="font-black">{providerName}</span></div>}{latestMessageAt && <div className="mt-3 flex items-center justify-between gap-3 text-[13px]"><span className="text-[#526078]">Last message</span><time dateTime={latestMessageAt} className="font-bold text-[#526078]">{new Intl.DateTimeFormat("en-GB", { day: "numeric", month: "short" }).format(new Date(latestMessageAt))}</time></div>}</div><div className="mt-4 border-t border-[#f0f2f5] pt-3 text-right text-sm font-black text-[#167d3c]">View job <span aria-hidden="true">→</span></div></Link>{conversationIds.length > 0 && <Link href={conversationHref} className="mt-3 block border-t border-[#f0f2f5] pt-3 text-sm font-black text-[#167d3c]">{conversationIds.length === 1 ? "View conversation" : `View ${conversationIds.length} conversations`} <span aria-hidden="true">→</span></Link>}</article>; }

function getJobActivity(item: JobView) { if (item.state === "waiting_for_offers") return "Waiting for quotes"; if (item.state === "offers_received") return `${item.activeQuotes.length} ${item.activeQuotes.length === 1 ? "quote" : "quotes"} received`; if (item.state === "provider_selected_unpaid") return "Provider selected"; return item.statusLabel; }
