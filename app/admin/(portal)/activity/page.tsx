...
export default async function ActivityPage() {
  ...
  return <div className="mx-auto max-w-4xl"><h1 className="text-3xl font-black">Audit log</h1><p className="mt-1 text-[#657089]">Recent marketplace lifecycle events derived from live data.</p><ol className="mt-6 divide-y overflow-hidden rounded-2xl border bg-white">{events.map((event) => <li key={event.id} className="flex flex-wrap justify-between gap-3 p-5"><div><p className="font-black">{event.label}</p><p className="mt-1 text-sm text-[#526078]">{event.detail}</p></div><time className="text-sm text-[#657089]">{new Date(event.at).toLocaleString("en-GB")}</time></li>)}{!events.length && <li className="p-8 text-[#657089]">No marketplace activity yet.</li>}</ol></div>;
}
