...
export async function POST(request: Request) {
  ...
  const { data: booking, error: bookingLookupError } = await admin.from("marketplace_bookings").select("id,job_id,quote_id,customer_id,conversation_id,amount_pence,currency,payment_status,stripe_checkout_session_id,stripe_checkout_attempt_id,status,marketplace_jobs(public_token,status)").eq("id", bookingId).maybeSingle();
  ...
  if (sessionMismatch || attemptMismatch || booking.payment_status === "cancelled" || booking.status === "cancelled" || jobRelation?.status === "cancelled") { await admin.from("stripe_webhook_events").update({ status: "failed", error_message: "stale_or_cancelled_session" }).eq("stripe_event_id", event.id); return NextResponse.json({ received: true }); }
