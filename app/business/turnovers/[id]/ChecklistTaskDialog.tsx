...
export default function ChecklistTaskDialog({ turnoverId, sections }: { turnoverId: string; sections: string[] }) {
  ...
  return <>
    ...
    <dialog ref={dialogRef} className="w-[calc(100%-2rem)] max-w-lg rounded-xl p-0 shadow-xl backdrop:bg-[#071f49]/40">
      <form action={addTurnoverChecklistTask} className="grid gap-4 p-5 sm:p-6">
        <input type="hidden" name="turnoverId" value={turnoverId}/>
        <div className="flex items-start justify-between gap-4"><div><h2 className="text-xl font-extrabold text-[#071f49]">Add task</h2><p className="mt-1 text-sm text-[#657089]">Add a checklist item for this property.</p></div><button type="button" onClick={() => dialogRef.current?.close()} aria-label="Close" className="min-h-11 min-w-11 rounded-lg text-2xl text-[#657089]">×</button></div>
