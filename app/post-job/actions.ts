...
type DraftPayload = { category: string; service: string; answers: Record<string, string | number>; postcode: string; when: string; description: string; mobile: string; name: string; budget: string };
...
export async function submitConsumerJob(_state: { message: string }, form: FormData) {
  ...
  const payload: DraftPayload = { category, service, answers, postcode, when, description, mobile, name, budget };
