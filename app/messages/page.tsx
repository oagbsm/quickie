...
export default async function MessagesIndex({ providerOnly = false, searchParams }: { providerOnly?: boolean; searchParams?: Promise<{ conversation?: string; job?: string }> }) {
  ...
  const [{ data: jobs }, { data: profiles }, { data: quotes }, { data: messages }, { data: bookings }] = await Promise.all([
    jobIds.length ? admin.from("marketplace_jobs").select("id,service,service_subtype,postcode,status").in("id", jobIds) : Promise.resolve({ data: [] }),
    ...
    jobIds.length ? admin.from("marketplace_quotes").select("id,job_id,provider_id,bidder_user_id,amount_pence,status").in("job_id", jobIds).in("status", ["pending", "submitted", "selected", "accepted"]) : Promise.resolve({ data: [] }),
    ...
    jobIds.length ? admin.from("marketplace_bookings").select("job_id,conversation_id,provider_id,quote_id,payment_status,status,completion_status").in("job_id", [...new Set(jobIds)]) : Promise.resolve({ data: [] }),
  ]);
  const jobRows = (jobs || []) as Array<{ id: string; service?: string; service_subtype?: string; postcode?: string; status?: string | null }>;
  ...
  const resolvedProfileRows = isCustomer
    ? await Promise.all(profileRows.map(async (profile) => ({ ...profile, profile_photo_url: await resolveProviderPhotoUrl(admin, profile.profile_photo_url) })))
    : profileRows;
