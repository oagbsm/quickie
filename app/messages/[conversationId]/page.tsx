...
export default async function MessagesPage({ params, searchParams, providerOnly = false }: { params: Promise<{ conversationId: string }>; searchParams: Promise<{ error?: string }>; providerOnly?: boolean }) {
  ...
  const [{ data: job }, { data: messages }, { data: attachments }, { data: quote }, { data: booking }] = await Promise.all([
    ...
    admin.from("marketplace_bookings").select("provider_id,quote_id,status,payment_status,amount_pence,completion_status").eq("conversation_id", conversationId).maybeSingle(),
  ]);
  if (!job) notFound();
  ...
  const currentQuote = Boolean(booking && quote && conversationState.selectedProvider === conversationProviderId && conversationState.selectedQuoteId === quote.id);
  const quoteStatusLabel = booking && !currentQuote ? "Not selected" : booking?.payment_status === "paid" ? "Booked · Paid" : quote?.status;
  const canPay = Boolean(isCustomer && currentQuote && isAccepted && booking?.payment_status !== "paid" && !["cancelled", "completed"].includes(booking?.status || "") && !["cancelled", "completed"].includes(job?.status || "") && bookingState === "provider_selected_unpaid");
