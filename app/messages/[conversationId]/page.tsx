...
export default async function MessagesPage({ params, searchParams }: { params: Promise<{ conversationId: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  const [{ data: job }, { data: messages }, { data: quotes }, { data: profile }, { data: booking }] = await Promise.all([
    ...
    admin.from("marketplace_bookings").select("id,quote_id,conversation_id,amount_pence,payment_status,paid_at").eq("conversation_id", conversationId).maybeSingle(),
  ]);
  if (!job) notFound();
  ...
  const paymentAction = isCustomer && offerState === "booked" && booking?.payment_status !== "paid" && quote ? <form action={createMarketplaceCheckout} className="mt-4"><input type="hidden" name="token" value={job.public_token || ""} /><input type="hidden" name="quoteId" value={quote.id} /><input type="hidden" name="returnTo" value={`/messages/${conversationId}`} /><button className="min-h-11 rounded-xl bg-[#23a955] px-5 font-black text-[#061b3f]">Book &amp; pay</button><p className="mt-2 text-xs text-[#657089]">Secure payment with Stripe</p></form> : null;
  return <main className="min-h-screen bg-[#f7f8fa] text-[#061b3f]">{isProvider && !isCustomer ? <ProviderHeader /> : <MarketplaceHeader />}<section className="mx-auto max-w-2xl px-5 py-10"><Link href={isCustomer && job.public_token ? `/jobs/${job.public_token}` : "/work"} className="text-sm font-bold text-[#167d3c]">← Back to job</Link><div className="mt-5 rounded-3xl border border-[#e7ebef] bg-white p-6 sm:p-8"><p className="text-xs font-black uppercase tracking-[.14em] text-[#159548]">{title}</p><p className="mt-2 text-sm text-[#657089]">{location}</p><div className="mt-5"><h1 className="text-3xl font-black">Messages</h1><p className="mt-2 text-[#657089]">{profile?.business_name || profile?.display_name || "Local person"} · Provider</p></div>{offerHistory?.map((history) => <div key={history.id} className="mt-6 rounded-2xl border border-[#e7ebef] bg-[#f7f8fa] p-5"><p className="text-xs font-black uppercase tracking-[.14em] text-[#707b8d]">Offer · {history.status}</p><p className="mt-2 text-2xl font-black">£{Math.round(history.amount_pence / 100)}</p><p className="mt-1 text-sm text-[#526078]">{history.availability_text || "Flexible timing"}</p>{history.message && <p className="mt-3 text-sm text-[#39465b]">{history.message}</p>}</div>)}<section className="mt-6 rounded-2xl border border-[#dce7df] bg-[#f5fbf6] p-5"><p className="text-xs font-black uppercase tracking-[.14em] text-[#167d3c]">{isCustomer ? "Offer from" : "Your offer"} {profile?.business_name || profile?.display_name || "local person"}</p>{quote ? <><p className="mt-3 text-3xl font-black">£{Math.round(quote.amount_pence / 100)}</p><p className="mt-2 text-sm text-[#526078]">{quote.availability_text || "Flexible timing"}</p>{quote.message && <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-[#39465b]">{quote.message}</p>}{offerState === "booked" ? <>{booking?.payment_status === "paid" ? <p className="mt-4 rounded-xl bg-[#dff4e4] p-3 font-black text-[#167d3c]">✓ {isCustomer ? `Booked · £${Math.round(quote.amount_pence / 100)} paid` : `✓ Booking confirmed · £${Math.round(quote.amount_pence / 100)}`}</p> : <><p className="mt-4 rounded-xl bg-[#fff8e8] p-3 font-black text-[#8a5a00]">Accepted · ready to book</p>{paymentAction}</>}</> : offerState === "declined" ? <p className="mt-4 font-bold text-[#657089]">Offer not selected</p> : offerState === "withdrawn" ? <p className="mt-4 font-bold text-[#657089]">Offer withdrawn</p> : isCustomer ? <form action={chooseMarketplaceQuote} className="mt-4"><input type="hidden" name="token" value={job.public_token || ""} /><input type="hidden" name="quoteId" value={quote.id} /><input type="hidden" name="returnTo" value={`/messages/${conversationId}`} /><button className="min-h-11 rounded-xl bg-[#061b3f] px-5 font-black text-white">Accept £{Math.round(quote.amount_pence / 100)}</button></form> : <p className="mt-4 font-bold text-[#657089]">Pending</p>}</> : <p className="mt-3 text-sm text-[#657089]">No offer yet. Messages are open.</p>}</section><section className="mt-7 border-t border-[#e9edf1] pt-6"><h2 className="text-xl font-black">Messages</h2><div className="mt-4 grid gap-3">{messages?.length ? messages.map((message) => <div key={message.id} className={`max-w-[88%] rounded-2xl p-4 text-sm leading-6 ${message.sender_id === user.id ? "ml-auto bg-[#061b3f] text-white" : "bg-[#eef8f1]"}`}><p>{message.body}</p><time className="mt-2 block text-xs opacity-70">{new Intl.DateTimeFormat("en-GB", { dateStyle: "medium", timeStyle: "short" }).format(new Date(message.created_at))}</time></div>) : <p className="text-sm text-[#657089]">No messages yet.</p>}</div><form action={sendMarketplaceMessage} className="mt-4 flex gap-2"><input type="hidden" name="conversationId" value={conversationId} /><input type="hidden" name="returnTo" value={`/messages/${conversationId}`} /><input name="body" required maxLength={4000} placeholder="Write a message…" className="min-h-12 flex-1 rounded-xl border border-[#dbe1ea] px-4" /><button className="min-h-12 rounded-xl bg-[#23a955] px-5 font-black text-[#061b3f]">Send</button></form>{query.error && <p className="mt-3 text-sm font-bold text-red-700">Message could not be sent.</p>}</section></div></section></main>;
}
