...
export default async function CustomerConversationPanel({ conversationId }: { conversationId: string }) {
  ...
  const [{ data: provider }, { data: job }, { data: messages }, { data: attachments }, { data: quote }, { data: booking }] = await Promise.all([
    ...
    admin.from("marketplace_bookings").select("payment_status").eq("conversation_id", conversationId).maybeSingle(),
  ]);
  if (!job) notFound();
  ...
  return <section className="flex min-h-0 flex-1 flex-col"><div className="flex items-center gap-3 border-b border-[#e7ebef] px-5 py-4"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#eef8f1] text-lg font-black text-[#167d3c]" aria-hidden="true">{providerName.slice(0, 1).toUpperCase()}</div><div className="min-w-0"><p className="truncate font-black">{providerName}</p><p className="truncate text-sm font-bold text-[#526078]">{title} · {job.postcode}</p></div>{profileHref && <Link href={profileHref} className="ml-auto shrink-0 text-sm font-black text-[#167d3c]">View profile →</Link>}</div>{quote && <div className="flex flex-wrap items-center gap-x-4 gap-y-2 border-b border-[#edf0f3] bg-[#f5fbf6] px-5 py-3 text-sm"><span className="font-black">Quote {formatMarketplaceAmount(quote.amount_pence)}</span><span className="font-bold text-[#526078]">{booking?.payment_status === "paid" ? "Booked and paid ✓" : quote.status}</span>{quote.status === "submitted" && <form action={chooseMarketplaceQuote}><input type="hidden" name="token" value={job.public_token || ""} /><input type="hidden" name="quoteId" value={quote.id} /><input type="hidden" name="returnTo" value={returnTo} /><button className="font-black text-[#167d3c]">Accept quote</button></form>}{isAccepted && booking?.payment_status !== "paid" && <form action={createMarketplaceCheckout}><input type="hidden" name="token" value={job.public_token || ""} /><input type="hidden" name="quoteId" value={quote.id} /><input type="hidden" name="returnTo" value={returnTo} /><button className="rounded-lg bg-[#23a955] px-3 py-1.5 font-black text-[#061b3f]">Book &amp; pay</button></form>}</div>}<div className="min-h-0 flex-1 overflow-y-auto px-5 py-5"><div className="grid gap-3">{(messages || []).map((message) => <MarketplaceMessageBubble key={message.id} body={message.body} isMine={message.sender_id === user.id} attachments={byMessage.get(message.id)?.map((attachment) => ({ id: attachment.id, url: attachment.url, fileName: null }))} createdAt={message.created_at} />)}{!(messages || []).length && <p className="text-sm text-[#526078]">No messages yet.</p>}</div></div><div className="border-t border-[#e7ebef] px-5 py-4"><MessageComposer conversationId={conversationId} returnTo={returnTo} /></div></section>;
}
