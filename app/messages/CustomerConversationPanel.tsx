...
import { getMarketplaceConversationState, isMarketplaceConversationReadOnly } from "@/lib/marketplace/conversations";
...
export default async function CustomerConversationPanel({ conversationId }: { conversationId: string }) {
  ...
  const [{ data: provider }, { data: job }, { data: messages }, { data: attachments }, { data: quote }, { data: booking }] = await Promise.all([
    ...
    admin.from("marketplace_bookings").select("provider_id,quote_id,payment_status,status,completion_status,amount_pence").eq("conversation_id", conversationId).maybeSingle(),
  ]);
  ...
  const conversationState = await getMarketplaceConversationState(admin, conversationId);
  if (!job) notFound();
  ...
  const currentQuote = Boolean(booking && quote && conversationState.selectedProvider === providerId && conversationState.selectedQuoteId === quote.id);
  const quoteStatusLabel = booking && !currentQuote ? "Not selected" : booking?.payment_status === "paid" ? "Booked · Paid" : quote?.status;
  const canPay = Boolean(currentQuote && isAccepted && booking?.payment_status !== "paid" && !["cancelled", "completed"].includes(booking?.status || "") && !["cancelled", "completed"].includes(job?.status || "") && bookingState === "provider_selected_unpaid");
  return <section className="flex min-h-0 flex-1 flex-col"><div className="flex items-center gap-3 border-b border-[#e7ebef] px-5 py-4"><div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#eef8f1] text-lg font-black text-[#167d3c]" aria-hidden="true">{providerPhoto ? <img src={providerPhoto} alt="" className="h-full w-full object-cover" /> : providerName.slice(0, 1).toUpperCase()}</div><div className="min-w-0"><p className="truncate font-black">{providerName}</p><p className="truncate text-sm font-bold text-[#526078]">{title} · {job.postcode}</p></div>{profileHref && <Link href={profileHref} className="ml-auto shrink-0 text-sm font-black text-[#167d3c]">View profile →</Link>}</div>{quote && <div className="flex flex-wrap items-center gap-x-4 gap-y-2 border-b border-[#edf0f3] bg-[#f5fbf6] px-5 py-3 text-sm"><span className="font-black">{bookingCompleted ? "Booking completed" : `Quote ${formatMarketplaceAmount(quote.amount_pence)}`}</span><span className="font-bold text-[#526078]">{bookingCompleted ? `${formatMarketplaceAmount(booking?.amount_pence || quote.amount_pence)} paid` : quoteStatusLabel}</span>{!bookingCompleted && quote.status === "submitted" && !booking && <form action={chooseMarketplaceQuote}><input type="hidden" name="token" value={job.public_token || ""} /><input type="hidden" name="quoteId" value={quote.id} /><input type="hidden" name="returnTo" value={returnTo} /><button className="font-black text-[#167d3c]">Accept quote</button></form>}{canPay && <form action={createMarketplaceCheckout}><input type="hidden" name="token" value={job.public_token || ""} /><input type="hidden" name="quoteId" value={quote.id} /><input type="hidden" name="returnTo" value={returnTo} /><button className="rounded-lg bg-[#23a955] px-3 py-1.5 font-black text-[#061b3f]">Book &amp; pay</button></form>}</div>}<div className="min-h-0 flex-1 overflow-y-auto px-5 py-5"><div className="grid gap-3">{(messages || []).map((message) => <MarketplaceMessageBubble key={message.id} messageId={message.id} body={message.body} isMine={message.sender_id === user.id} canPromote={message.sender_id === user.id} promotedMessage={promotedMessageIds.includes(message.id)} promotedAttachments={promotedAttachmentIds} createdAt={message.created_at} />)}{!(messages || []).length && <p className="text-sm text-[#526078]">No messages yet.</p>}</div></div><div className="border-t border-[#e7ebef] px-5 py-4"><MessageComposer conversationId={conversationId} returnTo={returnTo} readOnly={conversationReadOnly} /></div></section>;
}
