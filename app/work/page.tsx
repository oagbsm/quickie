...
function shortJobDetail(answers: unknown) {
  ...
  return Object.entries(answers as Record<string, unknown>).filter(([, value]) => value !== null && value !== "").slice(0, 2).map(([key, value]) => `${key.replaceAll("_", " ")}: ${Array.isArray(value) ? value.join(", ") : String(value)}`).join(" · ");
}
...
export default async function WorkPage({ searchParams }: { searchParams: Promise<{ category?: string; sort?: string }> }) {
  ...
  if (provider.providerStatus === "action_required") redirect("/work/onboarding");
  if (["pending_review", "suspended"].includes(provider.providerStatus)) {
...
function ProviderStatusCard({ provider }: { provider: Awaited<ReturnType<typeof getMarketplaceProvider>> }) { if (!provider) return null; const status = provider.providerStatus; const stripe = provider.stripeStatus; const pending = status === "pending_review"; const title = pending ? "Your profile is being reviewed" : status === "suspended" ? "Provider access is suspended" : status === "approved" && stripe !== "ready" ? "One last step" : "Finish setting up your profile"; const body = pending ? "Your application is with Quickola for approval. You’ll be able to send quotes once your profile is approved." : status === "approved" ? "Your profile is approved, but payout setup is still needed before you can send quotes." : status === "suspended" ? "Quoting is disabled while this profile is suspended. Contact Quickola if you need help." : "Complete your basic profile and choose services before entering the provider workspace."; const review = status === "suspended" ? "Suspended" : pending ? "Awaiting approval" : status === "approved" ? "Approved" : "Setup incomplete"; const payout = stripe === "ready" ? "Ready" : stripe === "verification_pending" ? "Verifying" : "Not set up"; return <div className="rounded-3xl border border-[#e7ebef] bg-white p-8"><p className="text-xs font-black uppercase tracking-[.14em] text-[#159548]">PROVIDER STATUS</p><h1 className="mt-2 text-3xl font-black">{title}</h1><p className="mt-3 max-w-xl leading-7 text-[#657089]">{body}</p><div className="mt-6 flex flex-wrap gap-x-6 gap-y-2 text-sm font-black text-[#526078]"><span>Review · {review}</span><span>Payouts · {payout}</span></div>{pending && <div className="mt-6 rounded-2xl bg-[#f5fbf6] p-5 text-sm leading-6 text-[#39465b]"><p className="font-black text-[#167d3c]">What happens next?</p><p className="mt-2">We’ll let you know when your profile is approved.</p></div>}{status === "approved" && stripe !== "ready" && <Link href="/work/onboarding?setup=1" className="mt-7 inline-flex min-h-11 items-center rounded-xl bg-[#23a955] px-5 font-black text-[#061b3f]">Continue setup</Link>}{status !== "approved" && status !== "pending_review" && status !== "suspended" && <Link href="/work/onboarding" className="mt-7 inline-flex min-h-11 items-center rounded-xl bg-[#23a955] px-5 font-black text-[#061b3f]">Finish setup</Link>}</div>; }
