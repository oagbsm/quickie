...
function ProviderStatusCard({ provider }: { provider: Awaited<ReturnType<typeof getMarketplaceProvider>> }) { if (!provider) return null; const status = provider.providerStatus; const stripe = provider.stripeStatus; const pending = status === "pending_review"; const title = pending ? "Your profile is being reviewed" : status === "suspended" ? "Provider access is suspended" : status === "approved" && stripe !== "ready" ? "Finish your quote setup" : "Finish your provider setup"; const body = pending ? "Your application is with Quickola for approval. You’ll be able to send quotes once your profile is approved." : status === "approved" && stripe === "ready" ? "You’re ready to receive suitable local jobs." : status === "approved" ? "Your profile is approved, but payout setup is still needed before you can send quotes." : status === "suspended" ? "Quoting is disabled while this profile is suspended. Contact Quickola if you need help." : "Complete your basic profile and choose services."; return <div className="rounded-3xl border border-[#e7ebef] bg-white p-8"><p className="text-xs font-black uppercase tracking-[.14em] text-[#159548]">PROVIDER STATUS</p><h1 className="mt-2 text-3xl font-black">{title}</h1><p className="mt-3 max-w-xl leading-7 text-[#657089]">{body}</p><div className="mt-6 flex flex-wrap gap-3"><span className="rounded-full bg-[#f1f3f6] px-3 py-2 text-sm font-black">{pending ? "Quickola review — In progress" : status === "approved" ? "Quickola review — Approved" : "Quickola review — Suspended"}</span><span className="rounded-full bg-[#f1f3f6] px-3 py-2 text-sm font-black">Payouts — {stripe === "ready" ? "Ready" : "Needed"}</span></div>{pending && <div className="mt-6 rounded-2xl bg-[#f5fbf6] p-5 text-sm leading-6 text-[#39465b]"><p className="font-black text-[#167d3c]">What happens next?</p><p className="mt-2">We’ll let you know when your profile is approved.</p></div>}{status === "approved" && stripe !== "ready" && <Link href="/work/onboarding?setup=1" className="mt-7 inline-flex min-h-11 items-center rounded-xl bg-[#23a955] px-5 font-black text-[#061b3f]">Continue setup</Link>}</div>; }
