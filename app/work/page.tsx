...
export default async function WorkPage({ searchParams }: { searchParams: Promise<{ category?: string; sort?: string }> }) {
  ...
  if (!activeServices.size) return <WorkspaceShell><EmptyState title="Choose the services you offer to start seeing matching jobs." action="Set up my services" href="/work/profile" /></WorkspaceShell>;
  const supabase = await createSupabaseServerClient();
  const { data: matches } = await supabase.rpc("get_marketplace_opportunities");
  const ids = (matches || []).map((job: { id: string }) => job.id);
  const { data: jobs } = ids.length ? await admin.from("marketplace_jobs").select("id,public_token,service,service_subtype,postcode,budget_amount,requested_timing,optional_note,created_at,status").in("id", ids).order("created_at", { ascending: false }) : { data: [] };
