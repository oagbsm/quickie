...
export default async function ProviderPaymentsPage() {
  ...
  const { data: allocations } = bookingIds.length ? await admin.from("marketplace_payout_allocations").select("booking_id,gross_amount_pence,quickola_fee_pence,stripe_fee_pence,provider_net_pence,payout_status,stripe_payout_id,payout_eligible_at,stripe_available_on").in("booking_id", bookingIds) : { data: [] as Allocation[] };
  ...
  const processing = rows.filter(({ state }) => state.providerPayoutLabel === "Processing" || state.providerPayoutLabel === "Payment received").reduce((sum, row) => sum + (row.state.providerNet || 0), 0);
  const available = rows.filter(({ state }) => state.providerPayoutLabel === "Funds available" || state.providerPayoutLabel === "Payout pending").reduce((sum, row) => sum + (row.state.providerNet || 0), 0);
  const paidOut = rows.filter(({ state }) => state.providerPayoutLabel === "Paid out").reduce((sum, row) => sum + (row.state.providerNet || 0), 0);
  return <main className="min-h-screen bg-[#f7f8fa] text-[#061b3f]"><ProviderHeader /><section className="mx-auto max-w-5xl px-5 py-8 sm:px-8 sm:py-10"><p className="text-sm font-black uppercase tracking-[.12em] text-[#159548]">PROVIDER PAYMENTS</p><h1 className="mt-2 text-4xl font-black">Payments</h1><p className="mt-2 max-w-2xl text-[#657089]">A clear record of what customers paid and what you earned.</p><div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Metric label="Your earnings" value={money(totalEarnings)} /><Metric label="Processing" value={money(processing)} /><Metric label="Funds available" value={money(available)} /><Metric label="Paid out" value={money(paidOut)} /></div><section className="mt-8"><h2 className="text-2xl font-black">Payment history</h2><div className="mt-4 grid gap-4">{rows.map(({ booking, state }) => { const job = Array.isArray(booking.marketplace_jobs) ? booking.marketplace_jobs[0] : booking.marketplace_jobs; const status = state.providerPayoutLabel; return <article key={booking.id} className="rounded-2xl border border-[#e7ebef] bg-white p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><Link href={`/work/jobs/${booking.job_id}`} className="text-lg font-black text-[#167d3c]">{job?.service_subtype || job?.service || "Marketplace job"}</Link><p className="mt-1 text-sm text-[#657089]">{date(booking.paid_at || booking.created_at)}</p></div><span className="rounded-full bg-[#eef3f7] px-3 py-1.5 text-sm font-black">{status}</span></div><dl className="mt-5 grid gap-3 border-t border-[#edf0f3] pt-4 sm:grid-cols-2 lg:grid-cols-3"><Amount label="Customer paid" value={money(state.customerPaid)} /><Amount label="Quickola fee" value={money(state.quickolaFee)} /><Amount label="Stripe fee" value={money(state.stripeFee)} /><Amount label="Your earnings" value={money(state.providerNet)} /><Amount label="Status" value={status} />{state.stripeAvailableOn && status === "Processing" && <Amount label="Funds available" value={date(state.stripeAvailableOn)} />}</dl></article>; })}{!rows.length && <div className="rounded-2xl border border-dashed border-[#cfd8d2] bg-white p-8 text-center text-[#657089]">No verified paid marketplace bookings yet.</div>}</div></section></section></main>;
}
