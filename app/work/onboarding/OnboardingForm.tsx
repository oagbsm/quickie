...
import { refreshProviderPayoutSetup, saveProviderOnboarding, startProviderPayoutSetup, submitProviderApplication, updateProviderTerms, uploadProviderProfilePhoto } from "./actions";

type ServiceOption = { slug: string; name: string; jobs: { slug: string; name: string }[] };
type Props = { services: ServiceOption[]; initial: Record<string, unknown>; initialServices: string[]; readyToSubmit: boolean; basicProfileComplete: boolean; serviceAreaComplete: boolean; photoUrl: string | null; status: string; stripeStatus: string; emailVerified: boolean; actionReason?: string | null; error?: string; saved?: string; initialStep?: number };

export default function OnboardingForm({ services, initial, initialServices, readyToSubmit, basicProfileComplete, serviceAreaComplete, photoUrl, status, stripeStatus, emailVerified, actionReason, error, saved, initialStep = 1 }: Props) {
  ...
  const profileReady = hasBasicProfile && Boolean(initial.base_town);
  const quoteReadinessComplete = readyToSubmit;
  ...
  const payoutStarted = stripeStatus !== "not_started";
  ...
  const errorMessage = error === "save"
    ...
    : error === "stripe" || error === "stripe_status"
      ? "We couldn’t open Stripe verification. Please try again."
  ...
  return (
    <main className="min-h-screen bg-[#f7f8fa] text-[#061b3f]">
      ...
      <section className="mx-auto max-w-3xl px-5 py-8 sm:px-8"><p className="text-xs font-black uppercase tracking-[.15em] text-[#159548]">JOIN QUICKOLA</p><h1 className="mt-2 text-4xl font-black tracking-[-.05em]">Get started with local jobs</h1><p className="mt-2 text-[#657089]">Set up your profile, choose services, and get ready to send quotes.</p>
        ...
        {step === 3 && <p className="mt-5 text-sm text-[#526078]">Please also read our <Link href="/privacy-policy" className="font-black text-[#167d3c] underline">Privacy Notice</Link>.</p>}
        <form action={saveProviderOnboarding} className="mt-5 rounded-3xl border border-[#e7ebef] bg-white p-6"><input type="hidden" name="currentStep" value={step} /><input type="hidden" name="existingPhotoPath" value={String(initial.profile_photo_url || "")} />{selectedServices.map((value) => <input key={value} type="hidden" name="service" value={value} />)}
          ...
          {step === 3 && <section><h2 className="text-2xl font-black">{canSubmit ? "Almost done" : "Quote readiness"}</h2><p className="mt-2 text-[#657089]">{canSubmit ? "Everything is ready." : "Finish these last steps to submit your profile."}</p><div className="mt-5 grid gap-3 rounded-2xl bg-[#f5fbf6] p-5 text-sm font-bold"><p className={profileReady ? "text-[#167d3c]" : "text-[#8a5a00]"}>{profileReady ? "✓" : "○"} Profile</p><p className={hasServices ? "text-[#167d3c]" : "text-[#8a5a00]"}>{hasServices ? "✓" : "○"} Services</p>{!serviceAreaComplete && <p className="text-[#8a5a00]">○ Service area — <a href="/work/profile?edit=1" className="underline">choose where you work</a></p>}{!emailVerified && <p className="text-[#8a5a00]">○ Confirmed email — verify your email to continue</p>}<p className={termsAccepted ? "text-[#167d3c]" : "text-[#8a5a00]"}>{termsAccepted ? "✓" : "○"} Provider terms</p><p className={payoutReady ? "text-[#167d3c]" : "text-[#8a5a00]"}>{payoutReady ? "✓ Payouts ready" : payoutVerifying ? "○ Payouts — under review" : payoutActionRequired ? "○ Payouts — more information required" : payoutStarted ? "○ Payouts — setup incomplete" : "○ Payouts — setup required"}</p></div>{canSubmit && <p className="mt-5 text-sm leading-6 text-[#526078]">Submit your profile and we&apos;ll review it before you can start receiving jobs.</p>}{!canSubmit && <><div className="mt-5"><p className="font-bold">Profile photo</p>{photoPreview && <Image src={photoPreview} alt="Provider profile preview" width={96} height={96} unoptimized className="mt-2 h-24 w-24 rounded-2xl object-cover" />}{(!hasPhoto || photoPickerOpen) && <label className="mt-3 block font-bold"><span className="sr-only">Choose profile photo</span><input name="profilePhoto" type="file" accept="image/jpeg,image/png,image/webp" onChange={handlePhotoChange} disabled={photoSaving} className="block w-full rounded-xl border p-3" /><span className="mt-1 block text-xs text-[#657089]">{photoSaving ? "Uploading…" : photoError || "JPG, PNG or WebP up to 5MB."}</span></label>}{hasPhoto && !photoPickerOpen && <button type="button" onClick={() => { setPhotoPickerOpen(true); setPhotoError(""); }} className="mt-3 rounded-xl border px-3 py-2 text-sm font-black">Change photo</button>}</div><label className="mt-5 flex gap-3 rounded-xl border p-4 text-sm font-bold"><input type="checkbox" checked={termsAccepted} onChange={handleTermsChange} disabled={termsSaving} />I accept the <a href="/provider-terms">Quickola provider terms</a>{termsError && <span>{termsError}</span>}</label>{payoutVerifying && <><p className="mt-4 rounded-xl bg-[#fff8e8] p-4 text-sm font-bold text-[#8a5a00]">Stripe is reviewing your information. You don&apos;t need to resubmit unless Stripe asks for more information.</p><form action={refreshProviderPayoutSetup} className="mt-4"><button className="min-h-11 rounded-xl border border-[#dbe1ea] px-4 font-black">Check status</button></form></>}{payoutActionRequired && <p className="mt-4 rounded-xl bg-[#fff8e8] p-4 text-sm font-bold text-[#8a5a00]">Stripe needs additional information before you can receive payouts. Continue securely with Stripe.</p>}<div className="mt-5 flex flex-wrap gap-3">{!payoutReady && !payoutVerifying && <button type="submit" formAction={startProviderPayoutSetup} className="min-h-11 rounded-xl bg-[#23a955] px-4 font-black">{payoutActionRequired ? "Continue payout verification" : payoutStarted ? "Continue payout setup" : "Set up payouts"}</button>}</div></>}{canSubmit && <button type="submit" formAction={submitProviderApplication} className="mt-5 min-h-11 rounded-xl bg-[#23a955] px-4 font-black text-[#061b3f]">Submit for review</button>}</section>}
        </form>
      </section>
    </main>
  );
