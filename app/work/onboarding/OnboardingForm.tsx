...
export default function OnboardingForm({ services, initial, initialServices, profileComplete, photoUrl, status, stripeStatus, emailVerified, actionReason, error, saved, payouts, initialStep = 1 }: Props) {
  ...
  return (
    <main className="min-h-screen bg-[#f7f8fa] text-[#061b3f]">
      <header className="border-b border-[#e7ebef] bg-white"><div className="mx-auto flex min-h-16 max-w-5xl items-center justify-between gap-4 px-5 sm:px-8"><Link href="/work" className="text-xl font-black">Quickola</Link><ProviderHeaderNavigation pending={status === "pending_review"} /></div></header>
      <section className="mx-auto max-w-3xl px-5 py-8 sm:px-8"><p className="text-xs font-black uppercase tracking-[.15em] text-[#159548]">JOIN QUICKOLA</p><h1 className="mt-2 text-4xl font-black tracking-[-.05em]">Get started with local jobs</h1><p className="mt-2 text-[#657089]">Set up your profile, choose services, and get ready to send quotes.</p>
        {(!emailVerified || errorMessage || saved || payouts) && <p className="mt-5 rounded-xl bg-[#fff8e8] p-3 text-sm font-bold text-[#8a5a00]">{!emailVerified ? "Verify your authenticated email before continuing." : errorMessage || (saved ? "Your provider profile has been saved." : "Payout status refreshed.")}</p>}
        ...
        <div className="mt-7 flex gap-2"><button type="button" onClick={() => setStep(1)} className="rounded-full bg-white px-3 py-2 text-xs font-black ring-1 ring-[#e7ebef]">1 Basic profile</button><button type="button" disabled={!hasBasicProfile} onClick={() => setStep(2)} className="rounded-full bg-white px-3 py-2 text-xs font-black ring-1 ring-[#e7ebef]">2 Services</button><button type="button" disabled={!hasBasicProfile || !hasServices} onClick={() => setStep(3)} className="rounded-full bg-white px-3 py-2 text-xs font-black ring-1 ring-[#e7ebef]">3 Quote readiness</button></div>
        <form action={saveProviderOnboarding} className="mt-5 rounded-3xl border border-[#e7ebef] bg-white p-6"><input type="hidden" name="currentStep" value={step} /><input type="hidden" name="existingPhotoPath" value={String(initial.profile_photo_url || "")} />{selectedServices.map((value) => <input key={value} type="hidden" name="service" value={value} />)}
          ...
          {step === 2 && <section><h2 className="text-2xl font-black">What work do you do?</h2><div className="mt-5 grid gap-2">{services.map((service) => { const open = openCategories.includes(service.slug); return <div key={service.slug} className="rounded-2xl border"><button type="button" onClick={() => setOpenCategories((current) => open ? current.filter((item) => item !== service.slug) : [...current, service.slug])} className="flex min-h-14 w-full justify-between px-4 text-left font-black"><span>{service.name}</span><span>{service.jobs.filter((job) => selectedServices.includes(`${service.slug}|${job.slug}`)).length} selected</span></button>{open && <div className="grid gap-2 border-t p-3 sm:grid-cols-2">{service.jobs.map((job) => { const value = `${service.slug}|${job.slug}`; return <label key={value} className="flex gap-3 rounded-xl border p-3 text-sm font-bold"><input type="checkbox" checked={selectedServices.includes(value)} onChange={() => setSelectedServices((current) => current.includes(value) ? current.filter((item) => item !== value) : [...current, value])} />{job.name}</label>; })}</div>}</div>; })}</div><button className="mt-6 min-h-11 rounded-xl bg-[#23a955] px-5 font-black">See matching Maidenhead jobs</button></section>}
          {step === 3 && <section><h2 className="text-2xl font-black">Get ready to send quotes</h2>{fullyReady ? <div className="mt-5 rounded-2xl bg-[#f5fbf6] p-5"><p className="text-xl font-black text-[#167d3c]">You’re ready to go ✓</p><p className="mt-2 text-sm leading-6">Your account is approved and you’re ready to send quotes.</p><a href="/work" className="mt-5 inline-flex min-h-11 items-center rounded-xl bg-[#23a955] px-4 font-black">View available jobs</a><a href="/work/profile" className="ml-4 text-sm font-black text-[#167d3c]">Back to profile</a></div> : <><div className="mt-5 grid gap-3 rounded-2xl bg-[#f5fbf6] p-5 text-sm font-bold"><p>Profile photo — {hasPhoto ? "Ready" : "Needed"}</p><p>Provider terms — {termsAccepted ? "Ready" : "Needed"}</p><p>Quickola review — {status === "approved" ? "Approved" : status === "action_required" ? "Action needed" : "Not submitted"}</p><p>Payouts — {payoutReady ? "Ready" : "Needed"}</p>{missing && <p className="pt-2 text-[#8a5a00]">Still needed: {missing}</p>}</div><div className="mt-5"><p className="font-bold">Profile photo</p>{photoPreview && <Image src={photoPreview} alt="Provider profile preview" width={96} height={96} unoptimized className="mt-2 h-24 w-24 rounded-2xl object-cover" />}{(!hasPhoto || photoPickerOpen) && <label className="mt-3 block font-bold"><span className="sr-only">Choose profile photo</span><input name="profilePhoto" type="file" accept="image/jpeg,image/png,image/webp" onChange={handlePhotoChange} disabled={photoSaving} className="block w-full rounded-xl border p-3" /><span className="mt-1 block text-xs text-[#657089]">{photoSaving ? "Uploading…" : photoError || "JPG, PNG or WebP up to 5MB."}</span></label>}{hasPhoto && !photoPickerOpen && <button type="button" onClick={() => { setPhotoPickerOpen(true); setPhotoError(""); }} className="mt-3 rounded-xl border px-3 py-2 text-sm font-black">Change photo</button>}</div><label className="mt-5 flex gap-3 rounded-xl border p-4 text-sm font-bold"><input type="checkbox" checked={termsAccepted} onChange={handleTermsChange} disabled={termsSaving} />I accept the <a href="/provider-terms">Quickola provider terms</a>{termsError && <span>{termsError}</span>}</label><div className="mt-5 flex flex-wrap gap-3">{!payoutReady && <button type="submit" formAction={startProviderPayoutSetup} className="min-h-11 rounded-xl bg-[#23a955] px-4 font-black">{stripeStatus === "not_started" ? "Set up payouts" : "Continue payout setup"}</button>}{stripeStatus === "verification_pending" && <button type="submit" formAction={refreshProviderPayoutSetup} className="min-h-11 rounded-xl border px-4 font-black">Check payout status</button>}{canSubmit && <button type="submit" formAction={submitProviderApplication} className="min-h-11 rounded-xl bg-[#061b3f] px-4 font-black text-white">Submit for review</button>}</div></>}</section>}
        </form>
      </section>
    </main>
  );
