...
export default function OnboardingForm({ services, initial, initialServices, profileComplete, basicProfileComplete, photoUrl, status, stripeStatus, emailVerified, actionReason, error, saved, initialStep = 1 }: Props) {
  ...
  const quoteReadinessComplete = profileComplete && emailVerified && hasServices && hasPhoto && termsAccepted && payoutReady;
    ...
    <main className="min-h-screen bg-[#f7f8fa] text-[#061b3f]">
      ...
      <section className="mx-auto max-w-3xl px-5 py-8 sm:px-8"><p className="text-xs font-black uppercase tracking-[.15em] text-[#159548]">JOIN QUICKOLA</p><h1 className="mt-2 text-4xl font-black tracking-[-.05em]">Get started with local jobs</h1><p className="mt-2 text-[#657089]">Set up your profile, choose services, and get ready to send quotes.</p>
        ...
        <form action={saveProviderOnboarding} className="mt-5 rounded-3xl border border-[#e7ebef] bg-white p-6"><input type="hidden" name="currentStep" value={step} /><input type="hidden" name="existingPhotoPath" value={String(initial.profile_photo_url || "")} />{selectedServices.map((value) => <input key={value} type="hidden" name="service" value={value} />)}
          ...
          {step === 3 && <section><h2 className="text-2xl font-black">{canSubmit ? "Almost done" : "Quote readiness"}</h2><p className="mt-2 text-[#657089]">{canSubmit ? "Your profile is ready." : "Finish these last steps to start sending quotes."}</p><div className="mt-5 grid gap-3 rounded-2xl bg-[#f5fbf6] p-5 text-sm font-bold"><p className={profileReady ? "text-[#167d3c]" : "text-[#8a5a00]"}>{profileReady ? "✓" : "○"} Profile</p><p className={hasServices ? "text-[#167d3c]" : "text-[#8a5a00]"}>{hasServices ? "✓" : "○"} Services</p><p className={termsAccepted ? "text-[#167d3c]" : "text-[#8a5a00]"}>{termsAccepted ? "✓" : "○"} Provider terms</p><p className={payoutReady ? "text-[#167d3c]" : payoutVerifying ? "text-[#8a5a00]" : "text-[#8a5a00]"}>{payoutReady ? "✓ Payouts ready" : payoutVerifying ? "○ Payout details received — verification in progress" : "○ Payouts"}</p></div><div className="mt-5"><p className="font-bold">Profile photo</p>{photoPreview && <Image src={photoPreview} alt="Provider profile preview" width={96} height={96} unoptimized className="mt-2 h-24 w-24 rounded-2xl object-cover" />}{(!hasPhoto || photoPickerOpen) && <label className="mt-3 block font-bold"><span className="sr-only">Choose profile photo</span><input name="profilePhoto" type="file" accept="image/jpeg,image/png,image/webp" onChange={handlePhotoChange} disabled={photoSaving} className="block w-full rounded-xl border p-3" /><span className="mt-1 block text-xs text-[#657089]">{photoSaving ? "Uploading…" : photoError || "JPG, PNG or WebP up to 5MB."}</span></label>}{hasPhoto && !photoPickerOpen && <button type="button" onClick={() => { setPhotoPickerOpen(true); setPhotoError(""); }} className="mt-3 rounded-xl border px-3 py-2 text-sm font-black">Change photo</button>}</div><label className="mt-5 flex gap-3 rounded-xl border p-4 text-sm font-bold"><input type="checkbox" checked={termsAccepted} onChange={handleTermsChange} disabled={termsSaving} />I accept the <a href="/provider-terms">Quickola provider terms</a>{termsError && <span>{termsError}</span>}</label>{payoutVerifying && <p className="mt-4 rounded-xl bg-[#fff8e8] p-4 text-sm font-bold text-[#8a5a00]">Stripe is checking your information. You don&apos;t need to set up payouts again.</p>}{payoutActionRequired && <p className="mt-4 rounded-xl bg-[#fff8e8] p-4 text-sm font-bold text-[#8a5a00]">Payout setup needs more information.</p>}<div className="mt-5 flex flex-wrap gap-3">{!payoutReady && !payoutVerifying && <button type="submit" formAction={startProviderPayoutSetup} className="min-h-11 rounded-xl bg-[#23a955] px-4 font-black">{payoutActionRequired ? "Continue payout setup" : "Set up payouts"}</button>}{canSubmit && <button type="submit" formAction={submitProviderApplication} className="min-h-11 rounded-xl bg-[#061b3f] px-4 font-black text-white">Submit for review</button>}</div></section>}
        </form>
      </section>
    </main>
  );
