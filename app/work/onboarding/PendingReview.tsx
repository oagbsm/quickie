...
export default function PendingReview({ status, stripeStatus, profileComplete, servicesSelected, qualificationMissing, emailVerified }: Props) {
  ...
  return <StatusLayout><p className="text-xs font-black uppercase tracking-[.15em] text-[#159548]">PROVIDER APPLICATION</p><h1 className="mt-3 text-4xl font-black">Your application is being reviewed</h1><p className="mt-3 max-w-2xl leading-7 text-[#657089]">We’ve received your application. We’ll email you when it’s been reviewed or if we need anything else.</p><div className="mt-7 rounded-2xl bg-[#f5fbf6] p-5"><p className="text-xl font-black text-[#167d3c]">Awaiting Quickola approval</p><p className="mt-2 leading-7 text-[#39465b]">There’s nothing else you need to do right now.</p></div><div className="mt-7 grid gap-3 rounded-2xl border border-[#e7ebef] bg-white p-5 font-bold"><p className={profileComplete ? "text-[#167d3c]" : "text-[#8a5a00]"}>{profileComplete ? "✓" : "•"} Profile completed</p><p className={servicesSelected ? "text-[#167d3c]" : "text-[#8a5a00]"}>{servicesSelected ? "✓" : "•"} Services selected</p><p className={payoutReady || payoutInProgress ? "text-[#167d3c]" : "text-[#8a5a00]"}>{payoutReady || payoutInProgress ? "✓" : "•"} {payoutReady ? "Payouts set up" : "Payout setup still needed"}</p><p className="text-[#167d3c]">✓ Application submitted</p></div></StatusLayout>;
}
