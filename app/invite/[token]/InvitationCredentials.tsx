...
export default function InvitationCredentials({ email, token }: { email: string; token: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const { data, error } = await createSupabaseBrowserClient().auth.signUp({
      email, password,
      options: { emailRedirectTo: buildAbsoluteAppUrl(`/auth/callback?next=${encodeURIComponent(`/team/invite/${token}`)}`), data: { account_kind: "quickola_cleaner" } },
    });
    ...
    if (data.session) window.location.assign(`/team/invite/${token}`);
  ...
  return <div className="mt-7 grid gap-4">
    <Link href={`/business/sign-in?email=${encodeURIComponent(email)}&next=${encodeURIComponent(`/team/invite/${token}`)}`} className="flex min-h-12 items-center justify-center rounded-lg bg-[#071f49] font-extrabold text-white">Sign in to accept</Link>
