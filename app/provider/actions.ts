...
export async function submitProviderOffer(formData: FormData) {
  ...
  if (!user) redirect(`/auth/portal/sign-in?next=${encodeURIComponent(`/provider/jobs/${jobId}`)}`);
  ...
  if (error) redirect(`/provider/jobs/${jobId}?error=offer`);
  redirect(`/provider/jobs/${jobId}?sent=1`);
}
