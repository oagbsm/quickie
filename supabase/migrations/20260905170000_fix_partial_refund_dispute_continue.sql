create or replace function public.resolve_marketplace_dispute(target_dispute uuid, p_resolution_status text, p_resolution_code text, p_resolution_notes text default null)
...
begin
  ...
  for update;
  if result.id is null then raise exception 'dispute_not_resolvable'; end if;
  ...
  if p_resolution_status not in ('resolved_provider', 'resolved_customer') then raise exception 'invalid_resolution_status'; end if;

  if p_resolution_status = 'resolved_provider' then
    if current_booking.payment_status not in ('paid', 'partially_refunded')
    ...
    update public.marketplace_bookings
      set status = 'awaiting_customer_completion',
          ...
          provider_transfer_status = case when current_booking.provider_transfer_status = 'blocked' and current_booking.provider_transfer_error in ('customer_issue_reported', 'customer_refund_pending') then 'pending' else current_booking.provider_transfer_status end,
          provider_transfer_error = case when current_booking.provider_transfer_status = 'blocked' and current_booking.provider_transfer_error in ('customer_issue_reported', 'customer_refund_pending') then null else current_booking.provider_transfer_error end,
          updated_at = now()
    ...
    if current_booking.payment_status in ('cancelled', 'refund_pending') then raise exception 'booking_not_refundable'; end if;
    ...
    update public.marketplace_bookings
      set payment_status = case when current_booking.payment_status = 'paid' then 'refund_pending' else current_booking.payment_status end,
          ...
          provider_transfer_error = 'customer_refund_pending', payout_hold_status = 'held',
          payout_hold_reason = 'customer_resolution_refund', payout_hold_at = now(), updated_at = now()
      where id = current_booking.id;
  end if;
  return result;
end;
