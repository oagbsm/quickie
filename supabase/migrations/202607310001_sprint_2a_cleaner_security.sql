-- Sprint 2A: bind invitations to the worker's account at both write and accept time.

drop policy if exists "members manage invitations" on public.worker_invitations;
create policy "members manage invitations" on public.worker_invitations for all to authenticated
using (
  public.is_business_member(account_id)
  and exists(
    select 1 from public.workers worker
    where worker.id=worker_invitations.worker_id and worker.account_id=worker_invitations.account_id
  )
)
with check (
  public.is_business_member(account_id)
...
begin
  ...
  if worker.id is null or worker.account_id<>invitation.account_id then raise exception 'invitation_invalid'; end if;
