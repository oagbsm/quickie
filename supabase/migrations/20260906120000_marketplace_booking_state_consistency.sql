-- Keep quote history subordinate to the single current marketplace booking.
-- A paid booking identifies the only booked provider/quote; competing offers
-- remain in history but must not remain in an active selected state.
...
begin
  ...
  update public.marketplace_quotes set status=case when id=chosen.id then 'accepted' else 'declined' end, updated_at=now() where job_id=target_job.id and status in ('pending','submitted','selected','accepted','declined');
...
-- Repair persisted competing active quote states for already-paid bookings.
update public.marketplace_quotes q
set status='declined', updated_at=now()
where q.status in ('pending','submitted','selected','accepted')
  and exists (
    select 1 from public.marketplace_bookings b
    where b.job_id=q.job_id and b.payment_status='paid' and b.quote_id<>q.id
  );
